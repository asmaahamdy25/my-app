...
export const listMyOrder = (orderId, paymentResult) => async (
  dispatch,
  getState
) => {
  try {
...
export const listOrders = () => async (dispatch, getState) => {
  try {
    dispatch({ type: ORDER_ALLLIST_REQUEST })
    const userInfo = getState().userLogin.userInfo
    const config = {
      headers: {
        'Content-Type': 'application/json',
    ...
    dispatch({ type: ORDER_ALLLIST_SUCCESS, payload: data })
  } catch (error) {
    dispatch({
      type: ORDER_ALLLIST_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
